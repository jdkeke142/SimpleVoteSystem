...
public abstract class AbstractHasVoted {
    ...
    public AbstractHasVoted() {
        File file = new File(SimpleVoteSystemPlugin.getInstance().getDataFolder(), "config.yml");
        try {
            config = ConfigurationProvider.getProvider(YamlConfiguration.class).load(file);

            final Iterator<ProxiedPlayer>[] iterator = new Iterator[]{SimpleVoteSystemPlugin.getInstance().getProxy().getPlayers().iterator()};

            SimpleVoteSystemPlugin.getInstance().getProxy().getScheduler().schedule(SimpleVoteSystemPlugin.getInstance(), () -> {
                //It's here to allow dynamic enabling depending of the config.
                if (isEnabledWebsite()) {
                    Collection<ProxiedPlayer> players = SimpleVoteSystemPlugin.getInstance().getProxy().getPlayers();
    ...
    public int canVote(ProxiedPlayer player) {
        VotesManager votesManager = SimpleVoteSystemPlugin.getInstance().getVotesManager();
